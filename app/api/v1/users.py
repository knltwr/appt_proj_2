...
async def user_create(user: schemas_users.UserCreateRequest, db: Database = Depends(get_db)):
...
async def user_get(db: Database = Depends(get_db), payload: schemas_oauth2.TokenPayload = Depends(get_current_user)):
